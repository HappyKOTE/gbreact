          ...
          >
            <div className="position-relative">
              <div className="d-flex justify-content-between">
                ...
                <div className="text-muted">{value.chatLog[value.chatLog.length-1].timestamp}</div>
              </div>
      ...
      <div className="col-8 vh-100 pt-4 pb-4">
        <div className="bg-white h-100 rounded d-flex flex-column overflow-hidden">
          ...
          {chats[activeChat].chatLog.map((value, idx) => (
            <div
              className={(value.authorName === 'user') ? 'p-3 mb-2 alert alert-primary d-inline-block' : 'p-3 mb-2 alert alert-secondary d-inline-block'}
              key={idx}
            >
              <div className="d-flex justify-content-between">
                <div className="fw-bold me-3">{value.authorName}</div>
                <div>{value.message}</div>
                <div className="text-muted ms-3">{value.timestamp}</div>
              </div>
            </div>
          ))}
          ...
          <div className="border-top p-3 bg-light">
            <form className="m-0 p-0">
              <div className="input-group">
                <input type="text" className="form-control border-0 rounded bg-transparent" placeholder="написать сообщение" value={message}></input>
                <Button variant="primary" className="rounded-pill border-0 ms-2"><i className="bi bi-send"></i></Button>
              </div>
            </form>
          </div>
        </div>
